...
const MyCourses = () => {
  ...
  const handleSetRating = async (e, courseId, rating) => {
    ...
    if(!signed) {
      toast('please sign in to submit a rating', {icon: '⚠️'});
      return;
    }
  ...
  if(error) {
    ...
    return(
      <div className={myCoursesStyles.pageContainer}>
        <div className={myCoursesStyles.mainContainer}>
          <h1 className={myCoursesStyles.header}>My Courses</h1>
          <p className={myCoursesStyles.emptyText}>You haven't enrolled in any courses yet.</p>
        </div>
      </div>
    );
  }
      ...
      <div className={myCoursesStyles.mainContainer}>
        ...
        <div className={myCoursesStyles.grid}>
          {courses.map((course, index) => (
            <div key={course.id ?? index} className={myCoursesStyles.courseCard} style={{ animationDelay:`${index*100}ms`, animation:`fadeInUp 0.6s ease-out ${index*100}ms both`,}} role="button" tabIndex={0} onKeyDown={(e)=>{
