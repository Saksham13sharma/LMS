...
useEffect(()=>{
    try{
        localStorage.setItem('userCourseRating', JSON.stringify(ratings));
    }
    catch{
        //ignore
    }
...
// Fetch public courses
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/course/public`)
      ...
      .then(async (json) => {
        ...
        // Show all courses (removed filter that was excluding "top" courses)
  ...
  const submitRatingToServer = async (courseId, ratingValue)=>{
    try{
      ...
      const res= await fetch(`${API_BASE}/api/course/${encodeURIComponent(courseId)}/rate`,{
        method:"POST",
        headers,
        credentials:"include",
      ...
      if(!res.ok && !data.success){
        const msg=(data && (data.message || data.error)) || `Failed to submit rating (${res.status})`;
...
const filteredCourses = courses.filter(
    (course)=>
        course.name.toLowerCase().includes(searchQuery.toLowerCase()) || course.teacher.toLowerCase().includes(searchQuery.toLowerCase()) || course.category.toLowerCase().includes(searchQuery.toLowerCase())
);
...
//    navigate to course detail if logged in else
    ...
    <div className={coursePageStyles.pageContainer}>
      ...
      <div className={coursePageStyles.headerContainer}>
        <div className={coursePageStyles.headerTransform}>
            <h1 className={coursePageStyles.headerTitle}>LAERN & GROW</h1>
        </div>
        ...
        <div className={coursePageStyles.searchContainer}>
            ...
            <div className={coursePageStyles.searchInputContainer}>
                ...
                <input type="text" placeholder='Search courses by name, instructior or category... ' value={searchQuery} onChange={(e)=>{
                        ...
                        <div className={coursePageStyles.courseInfo}>
                          ...
                          <div className={coursePageStyles.ratingContainer}>
                            <div className={coursePageStyles.ratingStars}>
                              ...
                              >
                            <RatingStars courseID={course.id} userRating={userRating} avgRating={course.avgRating} totalRatings={course.totalRatings} onRate={handleRating} />
                              </div>
                            </div>
                          </div>
