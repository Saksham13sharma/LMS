    ...
    const loadCourse = async () => {
      try {
        ...
        if (data.success && data.course) {
          ...
          const lecturesData = Array.isArray(course.lectures) ? course.lectures : [];
          setFormData({
            name: course.name || "",
            teacher: course.teacher || "",
            image: { file: null, preview: course.image || "" },
            rating: course.rating || course.avgRating || 0,
            pricingType: course.pricingType || "free",
            price: {
              original: course.price?.original || "",
              sale: course.price?.sale || "",
            },
            overview: course.overview || "",
            totalDuration: {
              hours: course.totalDuration?.hours || "",
              minutes: course.totalDuration?.minutes || "",
            },
            totalLectures: course.totalLectures || lecturesData.length || "",
            lectures: lecturesData.map((lec, idx) => ({
              ...lec,
              id: lec._id || lec.id || `lecture-${idx}`,
              chapters: Array.isArray(lec.chapters) ? lec.chapters.map((ch, cidx) => ({
                ...ch,
                id: ch._id || ch.id || `chapter-${cidx}`,
              })) : [],
            })),
        ...
        toast.error("Error loading course: " + (err.message || "Unknown error"));
        navigate("/listcourse");
      } finally {
    ...
    <div className={addPageStyles.pageContainer}>
      ...
      <div className={addPageStyles.contentContainer}>
        <div className={addPageStyles.headerContainer}>
          ...
          <p className={addPageStyles.headerSubtitle}>Update your course details and manage content</p>
        </div>
                    ...
                    <div className={addPageStyles.chaptersList}>
                      {currentLecture.chapters.map((chapter) => (
                        ...
                        >
                          ...
                          <div
                            className={addPageStyles.chapterPreviewDuration}
                          >
                            {formatDuration(chapter.duration)}
                          </div>
                        </div>
