          ...
          <div className={navbarStyles.navbarContent}>
            <div className={navbarStyles.logoContainer}>
              ...
              <div className='leading-[0.95]' >
                <div className={navbarStyles.logoText}>SamEduHub</div>
              </div>
            </div>
            {/* desktop links */}
                ...
                {menuItems.map(({id,label,icon: Icon,path})=>{
                  ...
                  return (
                    <Link key={id} to={path} className={navbarStyles.desktopNavItem(isActive)}>
                      ...
                      <span className='lg:text-md xl:text-lg md:text-x5'>{label} 
                      </span>
                      {isActive && (
          ...
          <div className={navbarStyles.mobileMenu(isMenuOpen)}>
            <div className={navbarStyles.mobileMenuInner}>
              {menuItems.map(({id,label,icon: Icon,path})=>{
                  const isActive=location.pathname===path;
                  return (
                    <Link key={id} to={path} onClick={()=> setIsMenuOpen(false)} className={navbarStyles.mobileMenuItem(isActive)}>
