    ...
    <div className={dashboardStyles.pageContainer}>
        ...
        <div className={dashboardStyles.contentContainer}>
            ...
            <div className={dashboardStyles.headerContainer}>
                ...
                <p className={dashboardStyles.headerSubtitle}>Welcome back! Here's what's happening with your courses today.</p>
            </div>

            {error && (
                <div className={dashboardStyles.errorBanner} role="alert">{error}</div>
            )}
            {/* stats section */}
            <div className={dashboardStyles.statsGrid}>
                {stats.map((stat, index)=>{
                    ...
                    return (
                        <div key={stat.title} className={dashboardStyles.statCard} style={{animationDelay: `${index*100}ms`,}} >
            ...
            <div className={dashboardStyles.coursesContainer}>
                <div className={dashboardStyles.coursesHeader}>
                    ...
                    <div className={dashboardStyles.searchContainer}>
                        ...
                        <input type="text" placeholder="Search courses..." value={searchTerm} onChange={(e)=> setSearchTerm(e.target.value)} className={dashboardStyles.searchInput} />
                    </div>
                </div>
                {/* table */}
                    ...
                    <div className={dashboardStyles.table}>
                        <table>
                            ...
                            <tbody className={dashboardStyles.tableBody}>
                                {filteredCourses.map((course, index)=>(
                                    <tr key={course.id || `${index}`} className={dashboardStyles.tableRow} style={{animationDelay:`${index*50+400}ms`,}}>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                                            <div className='flex items-center'>
                                                <img src={course.image} alt={course.name} className={dashboardStyles.courseImage} />
                                                <div>
                                        ...
                                        <td className={dashboardStyles.studentsCell}><div className='felx items-center text-gray-700'>
                                            <span className={dashboardStyles.studentsText}>
                                                {course.students}
                                            </span>
                                        </div>
                                        </td>
                                        <td className={dashboardStyles.priceCell}>{course.price}
                                        </td>
                                        <td className="px-4 sm:px-6 py-3 sm:py-4">
                        ...
                        {filteredCourses.length === 0 && (
                            <div className={dashboardStyles.emptyState}>
                                ...
                                <button onClick={()=>setSearchTerm("")} className={dashboardStyles.clearButton}>Clear Search</button>
                            </div>
                        )}
