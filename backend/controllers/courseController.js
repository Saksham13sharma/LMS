...
//compute fields for totalleture , corse, duration 
// minute them and return an OBJ
...
// create image url frm stored value;
...
export const getCourses=async (req,res) =>{
    try{
        ...
        console.error('GetPCourses error: ',err);
        return res.status(500).json({
            success:false,
            error: 'server error'
        })
...
export const getCoursesById = async (req,res)=>{
    try{
        ...
        console.error('GetPCoursesById error: ',err);
        return res.status(500).json({
            success:false,
            error: 'server error'
        })    
...
export const createCourse=async (req,res)=>{
    try{
        ...
        console.error('GetPCourse error: ',err);
        return res.status(500).json({
            success:false,
            error: 'server error'
        })
...
export const updateCourse = async (req, res) => {
    try {
        ...
        // image handling: only update if new file is provided
        if (req.file) {
        ...
        if (body.overview) course.overview = body.overview;
        if (body.courseType) course.courseType = body.courseType;
        if (typeof body.rating !== 'undefined') course.rating = toNumber(body.rating, 0);
        if (body.pricingType) course.pricingType = body.pricingType;

        // parse price
        if (body.price || body["price.original"] || body["price.sale"]) {
            const priceParsed = parseJSONSafe(body.price) ?? (body.price || {});
            course.price = {
                original: toNumber(priceParsed.original ?? body["price.original"] ?? course.price?.original ?? 0),
                sale: toNumber(priceParsed.sale ?? body["price.sale"] ?? course.price?.sale ?? 0),
            };
        }

        // update total duration
        if (body.totalDuration || body["totalDuration.hours"] || body["totalDuration.minutes"]) {
            const durationParsed = parseJSONSafe(body.totalDuration) ?? (body.totalDuration || {});
            course.totalDuration = {
                hours: toNumber(durationParsed.hours ?? body["totalDuration.hours"] ?? course.totalDuration?.hours ?? 0),
                minutes: toNumber(durationParsed.minutes ?? body["totalDuration.minutes"] ?? course.totalDuration?.minutes ?? 0),
            };
        }
        ...
        computeDerivedFields(course.toObject());
        course.totalLectures = Array.isArray(course.lectures) ? course.lectures.length : 0;
    ...
    try{
        ...
        if(!Number.isFinite(rating) || rating <1 || rating >5 ){
            return res.status(400).json({
                success:false,
                message:'rating must be a number between 1'
            })
        }
...
export const getMyRating =async (req,res)=>{
    try{
        ...
        const my = (course.ratings || []).find(r=>String(userId)=== String(userId)) || null;
        return res.json({
            success: true,
